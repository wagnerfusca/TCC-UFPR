...
public class Departamento extends Pessoa {
    ...
    public Departamento() throws Exception{
      ...
      public ResultSet login(String usuario, String senha){
       try {
       sql = "select pessoa.nome_pessoa,pessoa.cod_pessoa,departamento.nome_departamento,departamento.cod_departamento,departamento.chefe_departamento from pessoa INNER JOIN departamento on departamento.cod_pessoa=pessoa.cod_pessoa where pessoa.usuario='"+usuario+"' and pessoa.senha='"+senha+"' ";
              return conexao.getSt().executeQuery(sql);      
        }catch (Exception ex){
            return null;
        }
