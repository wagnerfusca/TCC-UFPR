...
<script language="JavaScript" type="text/JavaScript">
	...
	if(document.form1.prof.value==""){
		alert("Erro! O campo PROFESSOR tem que ser preenchido!");
...
<p align="left" class="titulo_form">Envio de documento/formulário para professor</p>
...
<form action="salvaformInserir.jsp" name="form1" id="form1" method="post" enctype="multipart/form-data" onSubmit="return Verifica(this)" >
  <div>
    ...
    <div class="edit"><span class="letra_form">Selecione o professor</span>
      <select name="prof" class="input" id="prof" >
        <option value="">Selecione </option>
        <%
                    Object departamento =sessao.getAttribute("codDepartamento");
                    String codDepartamento = departamento.toString();
                    ResultSet rs2 = prof.BuscarProfessor(codDepartamento);
                                    while (rs2.next()){%>
        <option value="<%=rs2.getInt("cod_pessoa")%>"><%=rs2.getString("nome_pessoa")%></option>
        <%}%>
      </select>
      <input name="tipo" type="hidden" value="PROFESSOR">
      </input>
    </div>
    </p>
  </div>
