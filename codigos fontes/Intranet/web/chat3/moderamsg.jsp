         ...
         if (checks[x] != null)  //verifica se é diferente de nulo
    	  {
		    ...
		    chat.Pergunta item = (chat.Pergunta)ochat.getPerguntas().get(ncheck); //encontra o item e deleta.
		    // compara o item do objeto ochat com o itemlog do objeto olog. 
		    for (int i = olog.getPerguntas().size()-1; i > -1;i--)
		    {
		       chat.Pergunta itemlog = (chat.Pergunta)olog.getPerguntas().get(i);
		       if (itemlog.getTexto() != null)
			    { 
			      if (itemlog.getTexto().equals(item.getTexto()))
			       	{
	  		    	  olog.removePergunta(itemlog);   //Se o texto de olog for igual ao texto de ochat entao deleta olog
			    	}
			  }
			 }  
		     ochat.removePergunta(item);  				  // remove o item o chat
	      }
     }
       response.sendRedirect("limpamsg.jsp");     //vai para pagina limpamsg.jsp
   }
    else 
		response.sendRedirect("modchat.jsp");
%>
<html>
<head>
