...
<script language="JavaScript" type="text/JavaScript">
	...
	if(document.form1.coord.value==""){
		alert("Erro! O campo COORDENAÇÃO tem que ser preenchido!");
		document.form1.coord.focus();
		return false;
	}
...
<p align="left" class="titulo_form">Envio de documento/formulário para coordenação</p>
...
<form action="salvaformInserir.jsp" name="form1" id="form1" method="post" enctype="multipart/form-data" onSubmit="return Verifica(this)" >
  <div>
    ...
    <div class="edit"><span class="letra_form">Selecione a coordenação</span>
      <select name="coord" id="coord" class="input">
        <option vaiue="">Selecione </option>
        <%             ResultSet rs2 = coord.BuscarCoordenacao();
                                    while (rs2.next()){%>
        <option value="<%=rs2.getInt("cod_pessoa")%>"><%=rs2.getString("nome_coordenacao")%></option>
        <%}%>
      </select>
      <input name="tipo" type="hidden" value="COORDENACAO">
      </input>
    </div>
    </p>
  </div>
  ...
  <div class="edit">
    <p><span class="letra_form" class="input">Tipo Arquivo:</span>
